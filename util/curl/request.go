...
func (this *Request) Send(url string, method string) (*Response, error) {
	...
	if (method == "POST" || method == "PUT" || method == "PATCH") && this.PostData != nil {
		if jData, err := json.Marshal(this.PostData); err != nil {
			return nil, err
		} else {
			payload = bytes.NewReader(jData)
		}
	} else if (method == "POST" || method == "PUT" || method == "PATCH") && this.FormData != nil {
		fData := url2.Values{}
		for key, item := range this.FormData {
			fData.Set(key, item)
		}
		payload = bytes.NewReader([]byte(fData.Encode()))
	} else {
		payload = nil
	}
